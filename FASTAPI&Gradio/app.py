...
def add_quote_request(author, text, tags):
    payload = {"author": author, "text": text, "tags": tags}
    try:
        response = requests.post(f"{BACKEND_URL}/quotes", json=payload)
        if response.status_code == 200:
            return f"✅ Success! Added quote by {author}"
        return f"❌ Error: {response.text}"
    except Exception as e:
        return f"📡 Connection Error: Is the backend running at {BACKEND_URL}?"

def delete_quote_request(quote_id):
    try:
        response = requests.delete(f"{BACKEND_URL}/quotes/{quote_id}")
        return response.json().get("message", "Deleted!")
    except Exception as e:
        return f"❌ Failed to delete ID {quote_id}"
...
with gr.Blocks(theme=gr.themes.Soft(primary_hue="teal")) as app:
    ...
    # ── THE NEW CRUD TAB ───────────────────────────────────────────────────────
    with gr.Tab("Manage Database (CRUD)"):
        gr.Markdown("### ➕ Add New Quote")
        with gr.Row():
            new_author = gr.Textbox(label="Author")
            new_text = gr.Textbox(label="Quote Text")
            new_tags = gr.Textbox(label="Tags", value="general")
        
        add_btn = gr.Button("Save to Database", variant="primary")
        add_output = gr.Markdown()
        
        gr.Markdown("---")
        gr.Markdown("### 🗑️ Delete Quote")
        del_id = gr.Number(label="Enter Quote ID to Remove", precision=0)
        del_btn = gr.Button("Delete Permanently", variant="stop")
        del_output = gr.Markdown()

        add_btn.click(add_quote_request, 
                      inputs=[new_author, new_text, new_tags], 
                      outputs=add_output)
        
        del_btn.click(delete_quote_request, 
                      inputs=[del_id], 
                      outputs=del_output)
